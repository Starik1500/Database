...
@swag_from({
    ...
    'parameters': [
        {'name': 'playlist_id', 'in': 'path', 'type': 'integer', 'required': True}
    ],
    'responses': {
        200: {
...
def get_playlist_controller(playlist_id):
    ...
    if playlist:
        playlist_data = {'id': playlist.id, 'user_id': playlist.user_id}
...
@swag_from({
    ...
    'responses': {
        ...
        500: {'description': 'Failed to add song to playlist'}
    }
})
...
@swag_from({
    'tags': ['Playlists'],
    'description': 'Retrieve all songs (full objects) in a specific playlist',
    'parameters': [
        {'name': 'playlist_id', 'in': 'path', 'type': 'integer', 'required': True}
    ],
    'responses': {
        200: {'description': 'List of songs in playlist'},
        404: {'description': 'Playlist not found or no songs'},
        500: {'description': 'Unexpected data format'}
    }
})
