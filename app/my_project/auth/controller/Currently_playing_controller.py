...
@swag_from({
    ...
    'description': 'Get all currently playing entries',
    'responses': {200: {'description': 'List of currently playing entries'}}
})
...
@swag_from({
    'tags': ['Currently Playing'],
    'description': 'Get currently playing entry by ID',
    'parameters': [
        {'name': 'currently_playing_id', 'in': 'path', 'type': 'integer', 'required': True}
    ],
    'responses': {
...
@swag_from({
    'tags': ['Currently Playing'],
    'description': 'Add a new currently playing entry',
    'parameters': [
        {'name': 'body', 'in': 'body', 'required': True,
         'schema': {
             'properties': {
                 'is_played_now': {'type': 'boolean'},
                 'timestamp': {'type': 'string', 'example': 'Tue, 10 Oct 2023 13:00:00 GMT'},
                 'device': {'type': 'string'},
                 'song_id': {'type': 'integer'}
             },
             'required': ['is_played_now', 'timestamp', 'device', 'song_id']
         }}
    ],
    'responses': {
...
@swag_from({
    'tags': ['Currently Playing'],
    'description': 'Update an existing currently playing entry',
    'parameters': [
        {'name': 'currently_playing_id', 'in': 'path', 'type': 'integer', 'required': True},
        {'name': 'body', 'in': 'body', 'required': True,
         'schema': {
...
@swag_from({
    'tags': ['Currently Playing'],
    'description': 'Delete currently playing entry by ID',
    'parameters': [
        {'name': 'currently_playing_id', 'in': 'path', 'type': 'integer', 'required': True}
    ],
    'responses': {
